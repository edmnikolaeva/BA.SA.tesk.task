# Test Assignment – System & Business Analysis

This repository contains my solutions for the test assignment as a **System & Business Analyst**.  
The tasks include **process modeling, requirement specification, and API design**.



## ⚡ Key Points on Test Assignment Structure

### **Task 1 – BPMN**
The BPMN model reflects the workflow as described in the task, including noted questions and inconsistencies.
 [BPMN Diagram](https://github.com/edmnikolaeva/BA.SA.tesk.task/blob/main/задани1.jpg)

### **Task 2 – User Story / Use Case**
Use Cases and User Story follow with clear actors, preconditions, acceptance criteria and scenarios.  
An optional process diagram is included.
[Task2](https://github.com/edmnikolaeva/BA.SA.tesk.task/blob/main/BA.SA.test.task.by.NikolaevaED.pdf)

### **Task 3 – User Registration API**
OpenAPI specification and backend algorithm for user registration, including success and error responses, validation steps, and example requests/responses.  
A UML Activity Diagram demonstrates all steps in the backend algorithm executed before creating a user in the database.
[openAPI](https://github.com/edmnikolaeva/BA.SA.tesk.task/blob/main/задание3.1.yaml)
[UML.Activity.Diagram](https://github.com/edmnikolaeva/BA.SA.tesk.task/blob/main/задание3.2.jpg)

## ⚡ Additional key Points on API

- HTTP method and endpoint  
- Request and response parameters  
- Error codes and messages  
- Example requests/responses  


## ⚙️ Tools & Notations Used

- BPMN 2.0  
- UML Activity Diagram
- User Story / Use Case  
- OpenAPI / YAML
